...
namespace mathUtils{
    ...
    double normalCDF(double var);
		double bisection(double a, double b, std::function<double(double)> const& f, double epsilon = 1e-6, size_t maxIter = 1000);
		double secant(double x_0, double x_1, std::function<double(double)> const& f, double epsilon = 1e-6, size_t maxIter = 1000);
}
